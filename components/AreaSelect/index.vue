<template>
  <div class="area-select-wrap">
    <el-select
      ...
      :placeholder="placeholders[0] ? placeholders[0] : '请选择省'"
      :size="size"
      :disabled="disabled"
    >
      <el-option
    ...
    <el-select
      v-if="level >= 1"
      v-model="curCityCode"
      :placeholder="placeholders[1] ? placeholders[1] : '请选择市'"
      :size="size"
      :disabled="disabled"
    >
      <p v-if="!Object.keys(citys).length" class="area-select-empty">
        暂无数据
      </p>
      <el-option
    ...
    <el-select
      v-if="level >= 2"
      v-model="curAreaCode"
      :placeholder="placeholders[2] ? placeholders[2] : '请选择区'"
      :size="size"
      :disabled="disabled"
    >
      <p v-if="!Object.keys(areas).length" class="area-select-empty">
        暂无数据
      </p>
      <el-option
...
export default {
  ...
  data() {
    if (!this.data || !this.data['86']) {
      throw new Error(
        '[vue-area-linkage]: 需要提供地区数据，格式参考见：https://github.com/dwqs/area-data'
      )
    }
  ...
  methods: {
      ...
      if (this.level === 0) {
        ...
        if (this.defaults[1]) {
          if (this.isCode) {
            ...
            // this.$message({
            //   type: "warning",
            //   message: `城市 ${this.defaults[1]} 不存在于省份 ${
            //     this.defaults[0]
            //   } 中`
            // });
      ...
      if (this.level === 1) {
        ...
        if (this.defaults[2]) {
          if (this.isCode) {
            ...
            // this.$message({
            //   type: "warning",
            //   message: `县区 ${this.defaults[2]} 不存在于城市 ${
            //     this.defaults[1]
            //   } 中`
            // });
    ...
    setDefaultValue() {
      ...
      if (this.isCode) {
        ...
        // this.$message({
        //   type: "warning",
        //   message: `省份 ${this.defaults[0]} 不存在`
        // });
        provinceCode = find(
